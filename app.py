...
def image_similarity(a, b):
    ...
    try:
        ...
        return round(
            min(
                100.0,
                (len(good) / max(12, min(len(k1), len(k2)))) * 100
            ),
            2
        )
...
def home():
    ...
    lost = con.execute(
        "SELECT * FROM items WHERE item_type='Lost' ORDER BY id DESC"
    ).fetchall()

    found = con.execute(
        "SELECT * FROM items WHERE item_type='Found' ORDER BY id DESC"
    ).fetchall()
...
def google_callback():
    try:
        ...
        if not email:
            flash("Google account email was not received.")
            return redirect(url_for("login"))
        ...
        # If Google user is new, create an account automatically
        if not user:
...
def admin():
    ...
    lost = con.execute(
        "SELECT COUNT(*) c FROM items WHERE item_type='Lost'"
    ).fetchone()["c"]

    found = con.execute(
        "SELECT COUNT(*) c FROM items WHERE item_type='Found'"
    ).fetchone()["c"]
